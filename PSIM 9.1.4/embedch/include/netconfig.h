...
#if defined(_CHDLL_)
#include <dlfcn.h>
void *_ChNetconfig_handle = dlopen("libnetconfig.dl", RTLD_LAZY);
if(_ChNetconfig_handle == NULL) {
   fprintf(_stderr, "Error: dlopen(): %s\n", dlerror());
   fprintf(_stderr, "       cannot get _ChNetconfig_handle in netconfig.h\n");
   exit(-1);
} 
void _dlclose_netconfig(void) {
   dlclose(_ChNetconfig_handle);
}
atexit(_dlclose_netconfig);
#endif
